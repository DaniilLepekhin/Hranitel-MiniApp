  ...
  await telegramService.sendVideo(
    ...
    'https://t.me/mate_bot_open/9290',
    {
      caption:
        ...
        `И это ок.\n\n` +
        `В <b>КОДЕ ДЕНЕГ</b> мы как раз про другое.\n` +
        ...
        `▫️ Подари подружке доступ в <b>КОД ДЕНЕГ</b> —\n` +
...
export async function sendDay7CheckIn(userId: number, chatId: number) {
  ...
  await telegramService.sendPhoto(
    ...
    {
      caption:
        `<b>${firstName}, ты 7 дней с КОД ДЕНЕГ ✨</b>\n\n` +
...
export async function sendDay28Renewal(userId: number, chatId: number) {
  ...
  await telegramService.sendPhoto(
    ...
    {
      caption:
        `<b>Ты почти месяц с КОД ДЕНЕГ — 28 дней 🌙</b>\n\n` +
  ...
  await telegramService.sendPhoto(
    ...
    'https://t.me/mate_bot_open/9310',
    {
      caption:
        ...
        `Завтра завершается твой текущий период подписки в <b>КОД ДЕНЕГ</b>\n\n` +
...
export async function sendRenewalToday(userId: number, chatId: number) {
  ...
  await telegramService.sendPhoto(
    ...
    {
      caption:
        `${firstName}, информируем 🤍\n\n` +
        `Сегодня происходит автоматическое списание за следующий период подписки в <b>КОД ДЕНЕГ 💳</b>\n\n` +
  ...
  await telegramService.sendPhoto(
    ...
    'https://t.me/mate_bot_open/9312',
    {
      caption:
        ...
        `Если этот месяц был для тебя полезным и ты хочешь продолжить путь с <b>КОДОМ ДЕНЕГ</b> —\n` +
  ...
  await telegramService.sendPhoto(
    ...
    'https://t.me/mate_bot_open/9314',
    {
      caption:
        ...
        `Через <b>2 дня</b> закончится твой подарочный доступ к <b>КОДУ ДЕНЕГ.</b>\n\n` +
...
export async function sendGiftExpiry1Day(userId: number, chatId: number) {
  ...
  await telegramService.sendPhoto(
    ...
    'https://t.me/mate_bot_open/9315',
    {
      caption:
        `<b>🎁 Напоминаем</b>\n\n` +
        `Завтра заканчивается твой подарочный доступ к <b>КОДУ ДЕНЕГ.</b>\n\n` +
...
export async function handleUserShared(gifterTgId: number, recipientTgId: number, chatId: number) {
  ...
  await telegramService.sendPhoto(
    ...
    {
      caption:
        `<b>🎁 Стоимость подарка — 2.900 рублей</b>\n\n` +
  ...
  await telegramService.sendPhoto(
    ...
    'https://t.me/mate_bot_open/9316',
    {
      caption:
        ...
        `это <b>билет в клуб КОД ДЕНЕГ ⭐️</b>\n\n` +
        `Добро пожаловать 🤍\n` +
