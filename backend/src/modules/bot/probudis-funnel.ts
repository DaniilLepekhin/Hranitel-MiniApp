...
export async function sendProbudisDogrev(userId: string, chatId: number, utmData?: Record<string, string>): Promise<void> {
  try {
    ...
    const keyboard = new InlineKeyboard().webApp('Получить доступ в закрытый канал 👇🏼', paymentUrl);

    const message =
      `🎫 <b>Твой билет в КОД УСПЕХА. Глава: Пробуждение</b>\n\n` +
      `Информация о подписке на клуб:\n\n` +
      `👉🏼 1 месяц = 2000 ₽\n` +
      `👉🏼 В подписку входит полный доступ к клубу «Код Успеха»: марафон КОД ДЕНЕГ, обучение и мини-курсы по мягким нишам,\n` +
...
export async function sendProbudisSuccessStories(userId: string, chatId: number, utmData?: Record<string, string>): Promise<void> {
  try {
    ...
    const keyboard = new InlineKeyboard().webApp('Получить доступ', paymentUrl);
